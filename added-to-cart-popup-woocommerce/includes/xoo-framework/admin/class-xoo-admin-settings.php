...
class Xoo_Admin{
	...
	public function info_tab_data( $tab_id, $tab_data ){
		...
		echo $this->helper->get_outdated_section(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Output is fully escaped inside get_outdated_section()
		?>
		<p>
	...
	public function enqueue_scripts() {
		...
		//Select2 CSS file
	    wp_enqueue_style( 'select2-css', XOO_FW_URL . '/admin/assets/select2/select2.min.css', array(), '4.1.0-rc.0');
	    //select2js
	    wp_enqueue_script( 'select2-js',  XOO_FW_URL . '/admin/assets/select2/select2.min.js', 'jquery', '4.1.0-rc.0');
    ...
    foreach ( $tab_settings as $section_id => $settings ) {
        ...
        foreach ( $settings as $setting_id => $setting_data ) {
            ...
            // Ensure get_setting_html() is properly escaped inside itself
            $section_settings .= $this->get_setting_html( $id, $setting_data, $value );
        }

        if ( $section_settings ) {

            $safe_section_id = esc_attr( $tab_id . '_' . $section_id );

            $section_heading  = '<span id="' . $safe_section_id . '" class="xoo-asc-head xoo-asc-' . esc_attr( $section_id ) . '">';
            $section_heading .= esc_html( $section_data['title'] );
            $section_heading .= '</span>';

            if ( ! empty( $section_data['desc'] ) ) {
                $section_heading .= '<span class="xoo-asc-desc">' . esc_html( $section_data['desc'] ) . '</span>';
            }
        }
        ...
        $safe_class = esc_attr( implode( ' ', $section_class ) );

        $html .= '<div class="' . $safe_class . '">' . $section_heading . $section_settings . '</div>';
    }

    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- All internal content escaped already
    echo $html;
}
